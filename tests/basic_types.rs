...
fn test_slice() {
    ...
    let (output, len): (&[u8], usize) =
        bincode::borrow_decode_from_slice(&mut buffer[..8], bincode::config::standard()).unwrap();
...
fn test_str() {
    ...
    let (output, len): (&str, usize) =
        bincode::borrow_decode_from_slice(&mut buffer[..12], bincode::config::standard()).unwrap();
    assert_eq!(input, output);
    assert_eq!(len, 12);
}
...
fn test_array() {
    ...
    let (output, len): (&[u8], usize) =
        bincode::borrow_decode_from_slice(&mut buffer[..9], config).unwrap();
    assert_eq!(input, output);
    assert_eq!(len, 9);
}
